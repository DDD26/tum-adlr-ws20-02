...
def q0_random(*, q_start, q_end,
    ...
    if q_end is None:
        q_end = sample_q(robot=robot, n_samples=1)

    x_rp = sample_q(robot=robot, n_samples=n_random_points).transpose(1, 0, 2)
    if order_random:
        x_rp = path.order_path(x=x_rp, x_start=q_start, x_end=q_end)
    else:
        x_rp = np.concatenate((q_start, x_rp, q_end), axis=-2)

    return path.linear_connection(q=x_rp, n_waypoints=n_waypoints, infinity_joints=robot.infinity_joints)
...
def get_dummy_path(*, q_start=None, q_end=None,
                   ...
                   robot):
    if q_start is None:
        q_start, q_end = sample_q(robot=robot, n_samples=2)
