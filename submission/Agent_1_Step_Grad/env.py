...
class MPEnv(gym.Env):
    def __init__(self, objective, sup_dim, sup_lr):
        ...
        self.ob_space_dim = 2 * self.obj.opt_num
    ...
    def reset(self, random_start=False):
        ...
        observation = self.obj.ob_der_fun(self.pos).flatten().astype(np.float32)
        return observation
    ...
    def step(self, action):
        ...
        obs = self.obj.ob_der_fun(self.pos).flatten().astype(np.float32)
    ...
    def supervision(self):
        ...
        for i in range(positions.shape[0]):
            ...
            exp_obs[i] = current_der.astype(np.float32)
